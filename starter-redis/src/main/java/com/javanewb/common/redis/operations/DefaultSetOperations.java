...
class DefaultSetOperations<K, V> extends AbstractOperations<K, V> implements SetOperations<K, V> {
    ...
    public Set<V> distinctRandomMembers(K key, final long count) {
        ...
        final byte[] rawKey = rawKey(key);
        Set<byte[]> rawValues = execute((RedisCallback<Set<byte[]>>) connection -> new HashSet<>(connection.sRandMember(rawKey, count)), true);
    ...
    public Cursor<V> scan(K key, final ScanOptions options) {
        ...
        final byte[] rawKey = rawKey(key);
        return execute((RedisCallback<Cursor<V>>) connection -> new ConvertingCursor<>(connection.sScan(rawKey, options), this::deserializeValue), true);
