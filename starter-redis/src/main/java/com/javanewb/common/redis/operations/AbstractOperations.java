...
abstract class AbstractOperations<K, V> {
    ...
    @SuppressWarnings("unchecked")
    <K, V> Map<K, V> deserializeHashMap(Map<byte[], byte[]> entries) {
        ...
        for (Map.Entry<byte[], byte[]> entry : entries.entrySet()) {
            map.put((K) deserializeHashKey(entry.getKey()), (V) deserializeHashValue(entry.getValue()));
        }
    ...
    @SuppressWarnings({"unchecked"})
    <K> K deserializeHashKey(byte[] value) {
        if (hashKeySerializer() == null) {
            return (K) value;
        }
        return (K) hashKeySerializer().deserialize(value);
    }

    @SuppressWarnings("unchecked")
    <V> V deserializeHashValue(byte[] value) {
        if (hashValueSerializer() == null) {
            return (V) value;
        }
        return (V) hashValueSerializer().deserialize(value);
    }
}
